...
const MenuItems = ({ menuItemsData,index }) => {
  ...
  return (
    <li className="relative mx-1   cursor-pointer py-2 group" onMouseEnter={()=>setOpen(true)} onMouseLeave={()=>setOpen(false)}>
      ...
      <ul className={`${open?"z-[201]  block translate-x-0":"translate-x-[-20px] hidden  z-[-1]"}  absolute font-bold font-serif  bg-gray-50   p-3 w-fit px-1  rounded-md mt-2 ml-0  text-[14px] duration-700  ` } >
        {
